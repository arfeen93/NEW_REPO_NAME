...
import numpy as np
def test(model, test_data, attributes, tst_cls_lbl, device='cuda', zsl=True):
    model.eval()
    ...
    if zsl:
        ...
        #test_feat = torch.zeros(len(test_data), attributes[0].shape[0])
        #test_fix_embed = torch.zeros(classes, attributes[0].shape[0])
        ...
        #final_embeddings = np.concatenate((test_embeddings, test_fix_embed), axis = 0)
        acc_per_class = per_class_hits / per_class_samples
        acc_unseen = acc_per_class.mean(0)

        #import pdb;pdb.set_trace()
        return acc_unseen.item(), test_fix_embed, test_embeddings, target_pred
