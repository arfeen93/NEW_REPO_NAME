...
def manual_CE(predictions, labels):
    ...
    #lbl_margin = [torch.sum(all_margin[i:i+300]) for i in range(all_margin.shape[0], 300)]
    #import pdb;pdb.set_trace()
    #lbl_margin = torch.stack(lbl_margin)
    #loss = -torch.mean(torch.sum(labels * torch.log_softmax(predictions + all_margin,dim=1),dim=1))
    loss = -torch.mean(torch.sum(labels * torch.log_softmax(predictions, dim=1), dim=1))
    return loss


# def margin_in_CE(predictions, labels):
#     cos = nn.CosineSimilarity(dim = 0)
#     x = cos(attributes)
#     loss = - -torch.mean(torch.sum(torch.log(x)))
#     return loss
...
class UnitClassifier(nn.Module):
    def __init__(self, attributes, fc_margin, classes, samples_per_class, device='cuda', train = True):
        ...
        if train:
            ...
            #for i in classes:
                # margin = torch.zeros(300).to(device)
                # #sum = 0
                # for c in classes:
                #     if c != i:
                #         sim = cos(attributes[i.item()], attributes[c.item()])
                #         margin[c] = sim
                #         #print("similarity score for class {} with class {}".format(i.item(), c.item()), margin[c])
                # #self.margin = torch.Tensor(margin)
                # adaptve_margin = fc_margin(margin)
                #self.cls_margin[i.item()] = torch.sum(adaptve_margin)
                #print("class {} margin is :".format(i.item()), self.cls_margin[i.item()])
            # path = "/home/arfeen/CuMix_cluster34/saved_margins/"
            # if not os.path.isdir(path):
            #     os.makedirs(path)
            #     print("creating Mrgins directory")
            # else:
            #     print(" Margins directory exists")
            # m=0
            # margin_pth = os.path.join(path, "generated margin_call_init_{}.npy".format(m))
            # save(margin_pth, self.cls_margin)
            # m+=1
        # import pdb;
        # pdb.set_trace()
        # batch_samples_per_class = [self.samples_per_class[l.item()] for l in classes]
        # batch_samples_per_class = torch.Tensor(batch_samples_per_class).to(self.device)
        self.fc_margin = fc_margin
        for i,c in enumerate(classes):
            if train:
                attributes[c.item()] = attributes[c.item()].to(device)
                norm_attributes = attributes[c.item()] #+ fc_margin[c] * noise[i]  # (total_samples - self.samples_per_class[c])/total_samples
    ...
    def forward(self, x, labels, trn):
            ...
            # batch_sem = [self.attributes[l.item()] for l in labels]
            # batch_sem_repeat= list(itertools.chain.from_iterable(itertools.repeat(i, len(self.classes)) for i in batch_sem))
            # batch_sem_repeat = torch.stack(batch_sem_repeat)
            #
            # all_sem = [self.attributes[i] for i in range(len(self.classes))]
            # all_sem_repeat = list(itertools.chain.from_iterable(itertools.repeat(all_sem, len(labels))))
            # all_sem_repeat = torch.stack(all_sem_repeat)
            #
            # sim_score = self.cos(batch_sem_repeat, all_sem_repeat)
            # ind = [labels[i] + 300 *torch.tensor(i) for i in range(100)]
            # for j in ind:
            #     sim_score[j] = torch.tensor(0)
            #
            # sim_score = torch.reshape(sim_score, (len(labels), self.attributes[0].shape[0]))
            # #import pdb;pdb.set_trace()
            # margin = self.fc_margin(sim_score)

            # all_class_samples = [self.samples_per_class[i] for i in range(len(self.classes))]
            #
            # all_class_samples_repeat = list(itertools.chain.from_iterable(itertools.repeat(i, len(labels)) for i in all_class_samples))
            #
            # all_class_samples_repeat = torch.Tensor(all_class_samples_repeat).to(self.device)

            #batch_samples_per_class = [self.samples_per_class[l.item()] for l in labels]
            # batch_class_samples_repeat = list(itertools.chain.from_iterable(itertools.repeat(i, len(self.classes)) for i in batch_samples_per_class))
            # batch_samples_per_class = torch.Tensor(batch_class_samples_repeat).to(self.device)
            # samples = torch.reshape(batch_samples_per_class, (len(labels), self.attributes[0].shape[0]))
            #samples = torch.reshape(all_class_samples_repeat, (len(labels), self.attributes[0].shape[0]))
            # import pdb;
            # pdb.set_trace()
            # margin = self.fc_margin(samples)
            # o = self.fc(x)
            #import pdb;pdb.set_trace()
            #return o, margin
        #else:

        #print("margin variable:", self.fc_margin)
        #print("margin:", self.fc_margin)
        if trn:
...
class CuMix:
    ...
    def __init__(self, seen_classes, unseen_classes, attributes, configs, samples_per_class, tst_samples_per_class,
        ...
        if dg_only:
            ...
            # self.fc_margin = nn.Sequential(OrderedDict([
            #     ('fc_1', nn.Linear(attSize, 512)),
            #     ('batch norm1', nn.BatchNorm1d(512)),
            #     ('relu_1', nn.ReLU()),
            #     ('fc_2', nn.Linear(512, attSize)),
            #     ('batch norm2', nn.BatchNorm1d(attSize)),
            #     ('relu_2', nn.ReLU()),
                # ('fc_3', nn.Linear(512, 512)),
                # ('relu_3', nn.ReLU()),
                # ('batch norm3', nn.BatchNorm1d(512)),
                # ('fc_4', nn.Linear(512, attSize)),
                # ('relu_4', nn.ReLU()),
                # ('batch norm4', nn.BatchNorm1d(attSize))
                #]))

            # import pdb;
            # pdb.set_trace()
            #margin = torch.zeros(attSize)
            ...
            #margin = torch.tensor(margin, requires_grad=True)
            self.fc_margin = nn.Parameter(torch.tensor(margin), requires_grad=True)
            #self.fc_margin = nn.Parameter(margin)
    ...
    def save(self, dict):
        ...
        #dict['fc_margin'] = self.fc_margin.state_dict()
    ...
    def load(self, dict):
        ...
        #self.fc_margin.load_state_dict((dict['fc_margin']))
    ...
    def zero_grad(self):
        ...
        #if self.fc_margin.grad is not None:
        #self.fc_margin.grad.data.zero_()
    ...
    def fit(self, data, e):
        ...
        # optimizer_margin = optim.SGD(self.fc_margin.parameters(), lr=self.lr * scale_lr, momentum=0.9,
        #                                   weight_decay=self.decay, nesterov=self.nesterov)

        optimizer_margin = optim.SGD([self.fc_margin], lr=self.lr * scale_lr, momentum=0.9,
                                     weight_decay=0, nesterov=self.nesterov)

        # scheduler_margin = MultiStepLR(optimizer_margin, milestones=[3, 8], gamma=0.1)
        # scheduler_mixratio = MultiStepLR(optimizer_mixratio, milestones=[3, 8], gamma=0.1)
        # scheduler_zsl = MultiStepLR(optimizer_zsl, milestones=[3, 8], gamma=0.1)
        # scheduler_net = MultiStepLR(optimizer_net, milestones=[3, 8], gamma=0.1)
        #print("Epoch:{} lr:{} lr_net:".format(e, optimizer_mixratio.param_groups[0]['lr'], optimizer_net.param_groups[0]['lr']))
        ...
        for i, (inputs, _, domains, labels) in enumerate(dataloader):
            ...
            #preds, features, mixratio = self.forward(inputs, return_features=True)
            ...
            if self.end_to_end:
                ...
                #img_mixupratio_loss = self.soft_cce(img_mixup_ratio_pred, ratio_vec_gt)
                ...
                #img_mratio_loss += img_mixupratio_loss.item()
            ...
            # scheduler_zsl.step()
            # scheduler_mixratio.step()
            # scheduler_margin.step()
            del total_loss
            # if i==10:
                #break
        #print("fc margin parameters:", list(self.fc_margin.named_parameters()))
